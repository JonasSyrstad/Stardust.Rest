    ...
    class ServiceBuilder
    {
        ...
        private static List<ParameterWrapper> GetMethodParams(MethodInfo implementationMethod, IServiceLocator serviceLocator)
        {
            ...
            foreach (var parameterInfo in implementationMethod.GetParameters())
            {
                ...
                methodParams.Add(new ParameterWrapper { Name = parameterInfo.Name, Type = parameterInfo.ParameterType, In = @in?.InclutionType ?? InclutionTypes.Path });
            }
            return methodParams;
        }
       ...
       null
        ...
        private TypeBuilder CreateServiceType(Type interfaceType)
        {
            ...
            if (routePrefix != null)
            {
                ...
                var routePrefixCtor = typeof(RouteAttribute).GetConstructor(
                           BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                           null,
                           new Type[]{
                        typeof(string)
